    ...
    return (
        <>
            <HelmetProvider>
                <Helmet>
                    <title>
                        Top {category || "Job"} Jobs in Pakistan | Software, Development, Data & More - Aidifys.com
                    </title>
                    <meta
                        name="description"
                                ...
                                <section key={job._id} className='card border border-gray-300 rounded p-3 mb-4 hover:shadow-lg'>
                                    <Link to={`/job/${job?.slug}`} className='flex flex-row sm:flex-row items-start gap-4 p-1 sm:p-2 lg:p-3'>
                                        <div className='w-20 h-20 sm:w-32 sm:h-32 md:w-40 md:h-40 lg:w-44 lg:h-44 flex-shrink-0'>
                                            <img src={`${BASE_URL}${job?.image || job?.imageFilename}`} alt={job?.companyName} className="w-full h-full object-cover" />
                                        </div>
                                        <div>
                                            ...
                                            <h6 className='sm:text-sm lg:text-xl font-semibold'> {job?.skills && Array.isArray(job?.skills) ? job?.skills.join(', ') : job?.skills || ''}</h6>
